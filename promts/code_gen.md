...
3. **Весь исходный код приложения (HTML, CSS, JS, вспомогательные файлы данных) должен находиться внутри папки `src/`.** Если в утверждённой структуре проекта из code-plan.md папка `src` отсутствует, создай её в корне проекта с помощью `makedir` и размести все генерируемые файлы кода внутри `src/`. Внутренние относительные ссылки (подключение CSS, JS, изображений) скорректируй так, чтобы приложение работало корректно при таком расположении. Если `src/` уже описана в плане, строго следуй указанной там иерархии.

4. Создай структуру директорий и файлов внутри `src/` согласно разделу «4. Структура проекта» плана (используй `makedir`). Если план задаёт полные пути, начиная с `src/`, используй их как есть.
...
5. Сгенерируй код, строго следуя плану:
   ...
   Инструкции по коду:
    - **Все файлы писать в `src/`**, используя пути, сформированные на основе структуры проекта и правила из п.3.
    - index.html: семантическая вёрстка, подключение стилей и скриптов в порядке, указанном в плане, пути указывать относительно итогового расположения файла внутри `src/`.
...
6. Запиши каждый файл отдельно через write_file, используя полные пути с обязательным префиксом `src/` (или согласно утверждённой в плане структуре, уже содержащей `src/`).
